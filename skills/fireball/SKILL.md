---
name: fireball
description: Acompanha uma reunião ao vivo com o Fireball — grava, transcreve e atua como escrivão/secretária, tomando notas em tempo real enquanto a reunião acontece.
---
...
que está sendo dito em tempo real através do CLI `fireball` e decide o que anotar.
...
3. Para cada trecho novo:
   ...
   - Se implicar uma ação concreta (tarefa, follow-up, algo a criar/enviar em outro sistema —
     Tolaria, Linear, Slack, calendário, etc.), anote-a como tal:
     `fireball note <meeting_id> "Ação: <o que fazer, para quem, até quando>"`.
     Avise o usuário na conversa. **Não execute a ação de verdade** sem ele pedir — nem mesmo
     ações internas ao Tolaria.
...
4. Quando o usuário mandar executar uma dessas ações, faça-a com a tool apropriada — criar a
   nota no Tolaria seguindo as convenções do `AGENTS.md` daquele vault, abrir uma issue no
   Linear, mandar uma mensagem no Slack — e registre com `fireball note` o que foi feito.
...
- Ação concreta entra como nota começando com `Ação:` — é o que permite varrer as pendências
  no fim da reunião.
...
4. Compare a transcrição (`fireball transcript show <meeting_id>`, já reescrita pelo finalize) com as
   ...
   precisa ser pedido.
5. Revise com o usuário as ações anotadas (as linhas `Ação:` em `notes.md`) antes de encerrar.
